...
for epoch = 1:epochs
    ...
    if mod(epoch, 10) == 0
        ...
        total_errors_val(epoch/10) = error_train/1200;
...
% plotting the losses
figure(1);
plot(iteration_train, total_errors_train);
title('Error Plot over Epochs(train)');
xlabel('Epochs');
ylabel('Error');
grid on;

figure(2);
plot(iteration_val, total_errors_val);
title('Error Plot over Epochs (val)');
xlabel('Epochs');
ylabel('Error');
grid on;
